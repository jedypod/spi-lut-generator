...
def generate_1d_LUT_image(ramp_1d_path,
    ...
    ramp.open(ramp_1d_path, spec, mode="Create")

    data = np.ndarray(resolution*spec.nchannels, dtype=float)
    for i in range(resolution):
        value = float(i) / (resolution - 1) * (max_value - min_value) + min_value
...
def generate_1d_LUT_from_image(ramp_1d_path,
    ...
    # well yet.
    ramp_data = ramp.read_image(format='float').flatten()
...
def correct_LUT_image(transformed_lut_image,
    ...
    if width != lut_resolution * lut_resolution or height != lut_resolution:
        ...
        correct.open(corrected_lut_image, correct_spec, mode="Create")
